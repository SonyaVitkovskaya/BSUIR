...
def exponentiation(x, degree, p):
    ...
    for i in bit_degree:
        result = ((result**2)*x)%p if i == '1'else (result**2)%p
    return result 
...
def create_keys():
    ...
    while p == q: q = getStrongPrime(1024)
    n = p * q
    e_func = (p - 1)*(q - 1)
    e = 0
...
def message_decryption():
    ...
    print("Расшифрованное сообщение: ", str(exponentiation(int(c), int(d), int(n))))
...
def signature_decryption(s_m_tup):
    s, m = s_m_tup
    n, e = read_file("open_key.txt").split(",")
    m2 = exponentiation(int(s), int(e), int(n))
    print("s = ", s)
    if m2 == m: print("ЭЦП соответствует")
    else: print("ЭЦП не соответствует")
    
create_keys()
message_encryption()
message_decryption()
signature_decryption(signature_encryption())