...
def correct_weights(W_ih, W_ho, l_rate, diff, state, grads, a = 0.1):
    delta_o = diff * derivative_active_func(np.dot(np.atleast_2d(state[1]), W_ho))
    delta_h = np.dot(delta_o, W_ho.T) * derivative_active_func(np.dot(np.atleast_2d(state[0]), W_ih))
    grad_o = np.dot(np.atleast_2d(state[1]).T, np.atleast_2d(delta_o))
    grad_h = np.dot(np.atleast_2d(state[0]).T, np.atleast_2d(delta_h)) 
    new_Who = W_ho - (l_rate * grad_o) # + a*grads[1])
    new_Wih = W_ih - (l_rate * grad_h) # + a*grads[0])
    return new_Wih, new_Who, [grad_h, grad_o]
...
def train(network_state, l_rate, max_error, l_matrix, e_matrix, W_ih, W_ho, scale):
    ...
    while error > max_error:
        ...
        for row_index in range(len(l_matrix)):
            ...
            W_ih, W_ho, grads = correct_weights(W_ih, W_ho, l_rate, diff, network_state, grads)
...
def main():
    ...
    choice = int(input("Выберите последовательность "))
    ...
    if add_seq_len > 0:
        ...
        elif choice == 3:  seq += power(seq[-1:][0], add_seq_len)
        elif choice == 4:  seq += periodic(seq[-1:][0], add_seq_len)
